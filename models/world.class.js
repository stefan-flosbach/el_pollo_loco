  ...
  checkCollisions() {
    ...
    this.level.enemies.forEach((enemy) => {
      ...
      if (
        ...
        this.character.hasStomped = true;
        return;
      }

      // Schaden mit Invincibility
    ...
    // Tote Chickens entfernen

    this.level.enemies = this.level.enemies.filter((enemy) => {
      return !(enemy instanceof Chicken && enemy.shouldBeRemoved());
    });
    ...
    this.level.enemies.forEach((enemy) => {
      ...
      // Endboss berührt Pepe → sofort tot
      ...
      this.level.enemies.forEach((enemy) => {
        if (
          ...
          this.statusBarEndboss.setPercentage(
            Math.max(0, 100 - enemy.hitCount * 20),
          );
        }
      });
    });
